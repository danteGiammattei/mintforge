...
export default function RouletteWheel({ betCoin, onResult, disabled, t }) {
  ...
  const drawW = useCallback((angle) => {
    ...
    br.addColorStop(0, "#5a3a14"); br.addColorStop(.4, "#8a5a20");
    br.addColorStop(.6, "#3a2410"); br.addColorStop(1, "#5a3818");
    ...
    for (let i = 0; i < 48; i++) {
      ...
      ctx.strokeStyle = maj ? "#f0c850" : "#7a5818";
    ...
    for (const sec of secs) {
      ...
      hg.addColorStop(0, "rgba(255,255,255,.06)"); hg.addColorStop(1, "rgba(0,0,0,.1)");
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.arc(cx, cy, R, a0, a1); ctx.closePath();
    ...
    hg2.addColorStop(0, "#5a3818"); hg2.addColorStop(1, "#0e0804");
    ctx.fillStyle = hg2; ctx.fill();
    ctx.strokeStyle = "#d4a017"; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = "#d4a017"; ctx.font = "14px serif";
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText("⚒", cx, cy + 1);

    // Pointer arrow
    ctx.beginPath(); ctx.moveTo(cx - 10, 2); ctx.lineTo(cx + 10, 2); ctx.lineTo(cx, 22); ctx.closePath();
    ctx.fillStyle = "#f0c850"; ctx.fill();
    ctx.strokeStyle = "#0a0604"; ctx.lineWidth = 1.5; ctx.stroke();
